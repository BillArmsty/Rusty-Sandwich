...
use revm::{ primitives::{ Address as rAddress, Bytecode, U256 as rU256 }, EVM };
use std::str::FromStr;

// Setup rustysandwich for current fork factory by injecting rustysandwich
...
pub fn attach_rustysandwich_module(fork_factory: &mut ForkFactory) {
    inject_rustysandwich_code(fork_factory);

    // Get balance mapping of rustysandwich contract inside of weth contract
    let slot: U256 = ethers::utils
        ::keccak256(
            abi::encode(
                &[
                    abi::Token::Address(rustysandwich_address().0.into()),
    ...
    let value = rustysandwich_starting_balance();

    fork_factory
        .insert_account_storage(constants::get_weth_address().0.into(), slot.into(), value.into())
        .unwrap();
}
...
pub fn inject_sando(fork_factory: &mut ForkFactory, starting_weth_balance: U256) {
    ...
    let slot: U256 = ethers::utils
        ::keccak256(
            abi::encode(&[abi::Token::Address(sandwich.0.into()), abi::Token::Uint(U256::from(3))])
        )
        .into();
...
// Add bytecode to rustysandwich address
//
// Arguments:
// `&mut fork_factory`: mutable reference to `ForkFactory` instance to inject
//
// Returns: This function returns nothing
fn inject_rustysandwich_code(fork_factory: &mut ForkFactory) {
    // setup rustysandwich contract
    let account = revm::primitives::AccountInfo::new(
        rU256::from(0),
        0,
        Bytecode::new_raw(constants::get_rustysandwich_code().0)
    );
    fork_factory.insert_account_info(rustysandwich_address().0.into(), account);

    // setup rustysandwich contract controller
    let account = revm::primitives::AccountInfo::new(
        parse_ether(69).unwrap().into(),
        0,
        Bytecode::default()
    );
    fork_factory.insert_account_info(rustysandwich_controller_address().0.into(), account);
}
...
pub fn get_amount_out_evm(
    ...
    let output: Bytes = match result {
        ...
        ExecutionResult::Revert { output, .. } => {
            return Err(SimulationError::EvmReverted(output));
        }
    ...
    let tokens = abi
        ::decode(&vec![ParamType::Uint(128), ParamType::Uint(128), ParamType::Uint(32)], &output)
        .unwrap();
...
pub fn get_balance_of_evm(
    ...
    match erc20.decode_output("balanceOf", &output) {
        ...
        Err(e) => {
            return Err(SimulationError::AbiError(e));
        }
    }
}
...
// Holds constant value representing rustysandwich contract address
pub fn rustysandwich_address() -> rAddress {
    rAddress::from_str("00000000000000000000000000000000F3370000").unwrap()
}

// Holds constant value representing rustysandwich caller
pub fn rustysandwich_controller_address() -> rAddress {
    rAddress::from_str("000000000000000000000000000000000420BABE").unwrap()
}

// Holds constant value representing rustysandwich weth starting balance
pub fn rustysandwich_starting_balance() -> U256 {
