...
use crate::utils::tx_builder::{ self, rustysandwich, SandwichMaker };

use super::{
    attach_rustysandwich_module,
    rustysandwich_address,
    rustysandwich_controller_address,
    rustysandwich_starting_balance,
    setup_block_state,
};
...
async fn juiced_quadratic_search(
    ...
    attach_rustysandwich_module(&mut fork_factory);
    ...
    loop {
        ...
        for bound in &intervals {
            let sim = tokio::task::spawn(
                evaluate_sandwich_revenue(
                    *bound,
                    ingredients.clone(),
                    next_block.clone(),
                    fork_factory.new_sandbox_fork()
...
fn sanity_check(
    ...
    for meat in ingredients.meats.iter() {
        ...
        evm.env.tx.transact_to = TransactTo::Call(
            rAddress::from_slice(&meat.to.unwrap_or_default().0)
        );
    ...
    let revenue = post_sandwich_balance.checked_sub(sandwich_start_balance).unwrap_or_default();

    // filter only passing meat txs
    let good_meats_only = ingredients.meats
        .iter()
...
/// Sandwich simulation using rustysandwich contract (modified router contract)
///
/// Arguments:
...
async fn evaluate_sandwich_revenue(
    ...
    let frontrun_data = match pool_variant {
        PoolVariant::UniswapV2 =>
            rustysandwich::build_swap_v2_data(
                frontrun_in,
        ...
        PoolVariant::UniswapV3 =>
            rustysandwich::build_swap_v3_data(
                frontrun_in.as_u128().into(),
                ingredients.target_pool.address,
                ingredients.startend_token,
                ingredients.intermediary_token
    ...
    evm.env.tx.caller = rustysandwich_controller_address();
    evm.env.tx.transact_to = TransactTo::Call(rustysandwich_address().0.into());
    ...
    let (_frontrun_out, backrun_in) = match pool_variant {
        PoolVariant::UniswapV2 => {
            match tx_builder::rustysandwich::decode_swap_v2_result(output.into()) {
                Ok(output) => output,
                Err(e) => {
        ...
        PoolVariant::UniswapV3 => {
            match tx_builder::rustysandwich::decode_swap_v3_result(output.into()) {
                Ok(output) => output,
                Err(e) => {
                    return Err(SimulationError::FailedToDecodeOutput(e));
                }
    ...
    let revenue = post_sandwich_balance
        .checked_sub(rustysandwich_starting_balance())
        .unwrap_or_default();
...
mod test {
    ...
    async fn create_test(fork_block_num: u64, pool_addr: &str, meats: Vec<&str>, is_v2: bool) {
        ...
        let pool = match is_v2 {
            true => {
                testhelper::create_v2_pool(
                    pool_addr.parse::<Address>().unwrap(),
                    &ws_provider
                ).await
            }
            false => {
                testhelper::create_v3_pool(
                    pool_addr.parse::<Address>().unwrap(),
                    &ws_provider
                ).await
            }
        ...
        let initial_db = utils::state_diff
            ::to_cache_db(
                &state,
