...
async fn main() -> Result<()> {
    ...
    log::info!("{}", format!("{}", utils::constants::get_banner().green().bold()));
    ...
    let all_pools = sync_dex(dexes.clone(), &client, current_block, None).await.unwrap();
    ...
    loop {
        let client = utils::create_websocket_client().await.unwrap();
        let mut bot = Bot::new(client, all_pools.clone(), dexes.clone()).await.unwrap();
