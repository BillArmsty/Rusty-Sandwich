        ...
        for (target_pool, recipes) in self.pending_sandwiches.iter() {
            ...
            tokio::spawn(async move {
                ...
                let initial_db = utils::state_diff
                    ::to_cache_db(&combined_state_diffs, fork_block, &client).await
                    .unwrap();
                ...
                let mut optimal_sandwich = match
                    make_sandwich::create_optimal_sandwich(
                        &raw_ingredients,
                        weth_balance,
                        &next_block,
                        &mut fork_factory,
...
pub async fn send_bundle(
    ...
    let raw_signed_frontrun_tx = utils::sign_eip1559(
        front_slice_request,
        &sandwich_maker.searcher_wallet
    ).await?;

    let raw_signed_meat_txs: Vec<Bytes> = recipe.meats
        .iter()
        .map(|meat| meat.rlp())
        .collect();
    ...
    let raw_signed_backrun_tx = utils::sign_eip1559(
        back_slice_request,
        &sandwich_maker.searcher_wallet
    ).await?;
    ...
    for relay in relay::get_all_relay_endpoints().await {
        ...
        tokio::spawn(async move {
            ...
            log::info!(
                "{:?} {}",
                recipe.print_meats(),
                format!("Bundle sent to {}", relay.relay_name).bold().white().on_black()
            );
            ...
            if relay.relay_name == "flashbots" {
                ...
                match (is_bundle_included, recipe.has_dust) {
                    (true, false) => {
                        let other_token = if
                            recipe.target_pool.token_0 != utils::constants::get_weth_address()
                        {
...
fn calculate_bribe_for_max_fee(
    ...
    let revenue_minus_frontrun_tx_fee = match
        recipe.revenue.checked_sub(U256::from(recipe.frontrun_gas_used) * target_block.base_fee)
    {
        Some(revenue) => revenue,
        None => {
            return Err(SendBundleError::FrontrunGasFeesNotCovered());
        }
    };
