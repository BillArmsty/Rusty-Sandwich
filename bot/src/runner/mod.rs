...
impl Bot {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        let mut mempool_stream = if
            let Ok(stream) = rpc_extensions::subscribe_pending_txs_with_body(&self.client).await
        {
            stream
        } else {
        ...
        while let Some(mut victim_tx) = mempool_stream.next().await {
            ...
            let state_diffs = if
                let Some(sd) = utils::state_diff::get_from_txs(
                    &self.client,
                    &vec![victim_tx.clone()],
            ...
            let sandwichable_pools = if
                let Some(sp) = utils::state_diff::extract_pools(&state_diffs, &all_pools)
            {
                sp
            } else {
                log::info!("{:?}", victim_tx.hash);
            ...
            let fork_block = Some(
                BlockId::Number(BlockNumber::Number(block_oracle.next_block.number))
            );

            // create evm simulation handler by setting up `fork_factory`
            let initial_db = utils::state_diff
                ::to_cache_db(&state_diffs, fork_block, &self.client).await
                .unwrap();
            ...
            for sandwichable_pool in sandwichable_pools {
                ...
                tokio::spawn(async move {
                    ...
                    let raw_ingredients = if
                        let Ok(data) = RawIngredients::new(
                    ...
                    // check if has dust
                    let other_token = if
                        optimal_sandwich.target_pool.token_0 != utils::constants::get_weth_address()
                    {
