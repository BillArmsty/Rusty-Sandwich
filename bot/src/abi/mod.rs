...
abigen!(Erc20, "src/abi/IERC20.abi", event_derives(serde::Deserialize, serde::Serialize));
abigen!(
    RustySandwich,
    "src/abi/IRustySandwich.abi",
    event_derives(serde::Deserialize, serde::Serialize)
);

pub fn get_self_destruct_byte_code(target: Address) -> Result<Bytes, ParseBytesError> {
    let mut raw_byte_code: String = fs
        ::read_to_string("src/abi/SelfDestruct.byte")
