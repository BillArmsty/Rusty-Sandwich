    ...
    pub fn new_pool_from_event(&self, log: Log, provider: Arc<Provider<Ws>>) -> Option<Pool> {
        match self.pool_variant {
            PoolVariant::UniswapV2 => {
                ...
                let (token_0, token_1, address, _) = if
                    let Ok(pair) = uniswap_v2_factory.decode_event::<
                        (Address, Address, Address, U256)
                    >("PairCreated", log.topics, log.data)
                {
                    pair
                } else {
                    return None;
                ...
                Some(Pool::new(address, token_0, token_1, U256::from(3000), PoolVariant::UniswapV2))
            }
            PoolVariant::UniswapV3 => {
                let uniswap_v3_factory = UniswapV3Factory::new(self.factory_address, provider);

                let (token_0, token_1, fee, _, address) = if
                    let Ok(pool) = uniswap_v3_factory.decode_event::<
                        (Address, Address, u32, u128, Address)
                    >("PoolCreated", log.topics, log.data)
                {
                    pool
                } else {
                    return None;
                ...
                Some(Pool::new(address, token_0, token_1, U256::from(fee), PoolVariant::UniswapV3))
            }
        }
    }
}
...
pub async fn sync_dex(
    ...
    for handle in handles {
        match handle.await {
            ...
            Err(join_error) => {
                return Err(PairSyncError::JoinError(join_error));
            }
        }
    }
        ...
        handles.push(
            tokio::spawn(async move {
                ...
                let logs = provider.get_logs(
                    &Filter::new()
                        .topic0(
                            ValueOrArray::Value(dex.pool_variant.pool_created_event_signature())
                        )
