...
use ethers::{ prelude::*, utils::{ Anvil, AnvilInstance } };

use crate::{ prelude::{ Erc20, Pool, PoolVariant, UniswapV3Pool }, types::BlockInfo };
...
pub async fn create_fork_ws(fork_block_num: u64) -> (Arc<Provider<Ws>>, AnvilInstance) {
    ...
    let _anvil = Anvil::new()
        .fork(
            format!("https://eth-mainnet.g.alchemy.com/v2/nijvYQzHc4Fej8kvRvdesJXT5CqZEXSo@{}", fork_block_num)
        )
        .port(port_num)
        .spawn();
    let ws_fork = Ws::connect(format!("ws://127.0.0.1:{}", port_num)).await.unwrap();
...
pub async fn create_v2_pool(pool_address: Address, client: &Arc<Provider<Ws>>) -> Pool {
    ...
    Pool::new(pool_address, token_0, token_1, U256::from(3000), PoolVariant::UniswapV2)
}
...
pub async fn create_v3_pool(pool_address: Address, client: &Arc<Provider<Ws>>) -> Pool {
    ...
    Pool::new(pool_address, token_0, token_1, U256::from(fee), PoolVariant::UniswapV3)
}
...
pub fn mutate_weth_balance(
    ...
    state.account(super::constants::get_weth_address()).store(key, val);
