...
pub fn get_searcher_wallet() -> LocalWallet {
    let searcher_private_key = std::env
        ::var("SEARCHER_PRIVATE_KEY")
        .expect("Required environment variable \"SEARCHER_PRIVATE_KEY\" not set");
    searcher_private_key.parse::<LocalWallet>().expect("Failed to parse private key")
}

// Get block number that sandwich contract was deployed in
pub fn get_sandwich_inception_block() -> U64 {
    let inception_block = std::env
        ::var("SANDWICH_INCEPTION_BLOCK")
...
pub async fn get_ws_provider() -> Provider<Ws> {
    let url = dotenv
        ::var("RPC_URL_WSS")
        .expect("Required environment variable \"RPC_URL_WSS\" not set");
    Provider::<Ws>::connect(&url).await.expect("RPC Connection Error")
}
...
pub fn poison_alert_webhook() -> String {
    dotenv
        ::var("POISON_ALERT_DISCORD_WEBHOOK")
