...
use revm::{ db::{ CacheDB, EmptyDB }, primitives::{ AccountInfo, Bytecode } };
...
pub fn extract_pools(
    ...
    let weth_state_diff = &state_diffs.get(&utils::constants::get_weth_address())?.storage;
    ...
    for pool in touched_pools {
        ...
        let storage_key = TxHash::from(
            ethers::utils::keccak256(
                abi::encode(&[abi::Token::Address(pool.address), abi::Token::Uint(U256::from(3))])
            )
        );
...
pub async fn to_cache_db(
    ...
    for (address, acc_diff) in state.iter() {
        ...
        let future = async move {
            let nonce = nonce_provider.get_transaction_count(addy, block_num).await?;
