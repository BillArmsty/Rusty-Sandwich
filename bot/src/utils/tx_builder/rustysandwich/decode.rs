...
// Decode the result of the rustysandwich contract's calculateSwapV2 function
pub fn decode_swap_v2_result(output: Bytes) -> Result<(U256, U256), AbiError> {
    let rustysandwich_contract = BaseContract::from(
        parse_abi(
            &[
    ...
    rustysandwich_contract.decode_output("calculateSwapV2", output)
}

// Decode the result of the rustysandwich contract's calculateSwapV3 function
pub fn decode_swap_v3_result(output: Bytes) -> Result<(U256, U256), AbiError> {
    ...
    rustysandwich_contract.decode_output("calculateSwapV3", output)
}
