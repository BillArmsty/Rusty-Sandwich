...
impl SandwichLogicV3 {
    ...
    pub fn new() -> Self {
        ...
        for x in 0..jump_label_names.len() {
            jump_labels.insert(jump_label_names[x].to_string(), start_offset + 5 * (x as u32));
        }
    ...
    pub fn create_payload_weth_is_input(
        ...
        let pool_key_hash = ethers::utils::keccak256(
            abi::encode(
                &[abi::Token::Address(token_0), abi::Token::Address(token_1), abi::Token::Uint(fee)]
            )
        );
