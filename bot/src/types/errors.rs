...
pub enum PairSyncError {
    #[error("Provider error")] ProviderError(#[from] ProviderError),
    #[error("Contract error")] ContractError(#[from] ContractError<Provider<Ws>>),
    #[error("ABI error")] ABIError(#[from] AbiError),
    #[error("Join error")] JoinError(#[from] JoinError),
    #[error("Pair for token_a/token_b does not exist in provided dexes")] PairDoesNotExistInDexes(
        H160,
        H160,
    ),
}
...
pub enum SendBundleError {
    #[error("Failed to sign transaction")] SigningError(#[from] WalletError),
    #[error("Max fee is less than next base fee")] MaxFeeLessThanNextBaseFee(),
    #[error("Negative miner tip")] NegativeMinerTip(),
    #[error("Failed to create bundle")] FailedToCreateBundle(),
    #[error("Failed to send bundle")] FailedToSendBundle(),
    #[error("Revenue does not cover frontrun gas fees")] FrontrunGasFeesNotCovered(),
}
...
impl fmt::Display for SimulationError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            SimulationError::ZeroOptimal() => { write!(f, "No optimal sandwich found") }
        }
    }
}
